document.addEventListener('DOMContentLoaded', () => {
    ...
    // Staging Area
    const stagingArea = document.getElementById('new-player-staging');
    ...
    async function addPlayerFromAPI(apiPlayer) {
        ...
        try {
            ...
            // Create Staging Player Object (Unsaved)
            const newPlayer = {
                id: playfabId, // Use PlayFab ID as temporary ID
                playfab_id: playfabId,
                name: currentNickname,
                tier: 'staging', // Special tier for staging
                source: 'api',
                original_name: displayName !== currentNickname ? displayName : null
            };

            // Add to staging
            stagingPlayers.push(newPlayer);
            renderStagingArea();

            nameInput.value = '';
            showToast(`⬇️ Dodano do poczekalni: ${currentNickname}`, 'success');
    ...
    function renderStagingArea() {
        ...
        if (stagingPlayers.length > 0) {
            stagingArea.classList.remove('hidden');
            stagingList.innerHTML = '';
            stagingPlayers.forEach((player, index) => {
            ...
            btn.addEventListener('click', async (e) => {
                ...
                if (existingPlayer) {
                    ...
                    showConfirmationModal(
                        '⚠️ Potencjalny Duplikat',
                        `Ten gracz (${currentName}) prawdopodobnie już jest na liście (Tier ${existingPlayer.tier}).<br>Powód detekcji: <strong>${reason}</strong><br>Czy na pewno chcesz dodać duplikat?`,
                        async () => {
    ...
    if (ctxStatsAuto) {
        ctxStatsAuto.addEventListener('click', () => {
            if (selectedPlayerId) {
                const player = findPlayerById(selectedPlayerId);
                if (player) {
                    const playfabId = player.playfab_id || player.id;
    ...
    function findPlayerById(id) {
        // Check staging first
        const stagingP = stagingPlayers.find(p => p.id == id);
        if (stagingP) return stagingP;
    ...
    function findAndRemoveLocal(id) {
        // Check staging
        const stagingIdx = stagingPlayers.findIndex(p => p.id == id);
        if (stagingIdx !== -1) {
            return stagingPlayers.splice(stagingIdx, 1)[0];
        }
    ...
    function handleDragStart(e) {
        ...
        draggedItem = this;
        // Handle staging tier
        sourceTier = this.dataset.tier === 'staging' ? 'staging' : parseInt(this.dataset.tier);
    ...
    columns.forEach(col => {
        ...
        col.addEventListener('drop', async (e) => {
            ...
            if (isAdmin && selectedPlayerIds.size > 0) {
                ...
                renderAllTiers();
                renderStagingArea(); // Update staging area if items were removed
    ...
    trashZone.addEventListener('drop', async (e) => {
        ...
        if (isAdmin && selectedPlayerIds.size > 0) {
            ...
            if (confirm(`Czy na pewno chcesz usunąć ${idsToDelete.length} graczy?`)) {
                ...
                for (const id of idsToDelete) {
                    if (sourceTier === 'staging') {
                        // Just removed from local staging array, no API call needed
                        console.log('Removed from staging (no API call)');
                    } else {
    ...
    // Duplicate findAndRemoveLocal REMOVED
